...
@qualify_app.command("deepspeed")
def qualify_deepspeed(
    run_dir: Annotated[
        Path,
        typer.Option(help="New or empty DeepSpeed qualification directory."),
    ],
    zero_stage: Annotated[
        int,
        typer.Option(help="DeepSpeed ZeRO stage; only stage 2 is supported."),
    ] = 2,
    backend: Annotated[
        str,
        typer.Option(help="Process-group backend; only gloo is supported."),
    ...
    try:
        ...
        result = run_deepspeed_qualification(
            run_root=run_dir,
            workload_profile=workload_profile,
        ...
        raise typer.Exit(code=EXIT_UNSUPPORTED) from error
    except DeepSpeedQualificationError as error:
        typer.echo(f"DeepSpeed qualification failed: {error}", err=True)
        raise typer.Exit(code=EXIT_QUALIFICATION_FAILED) from error
    typer.echo(result.final_verdict)
    typer.echo(f"Strategy: {result.strategy} via {result.implementation}")
    typer.echo(f"Backend/world size: {result.backend}/{result.world_size}")
