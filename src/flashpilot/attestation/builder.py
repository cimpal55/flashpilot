...
def emit_distributed_recovery_attestation(
    *,
    run_root: Path,
    result: DistributedQualificationResult,
) -> AttestationEmission:
    """Emit and verify an unsigned attestation for a passing FSDP restart."""

    if result.final_verdict != "VERIFIED" or not result.gate.passed:
        raise AttestationEmissionError("distributed attestation requires a passing exact Gate")
    if result.verified_persisted_bytes is None:
        raise AttestationEmissionError("distributed attestation requires post-Gate bytes")
    root = PathSandbox.create(run_root).root
    for relative in (
        EVIDENCE_MANIFEST_PATH,
        RECOVERY_ATTESTATION_PATH,
        ...
        if (root / relative).exists():
            raise AttestationEmissionError(f"attestation artifact already exists: {relative}")
    try:
        persisted = DistributedQualificationResult.model_validate_json(
            (root / result.result_path).read_text(encoding="utf-8")
        )
        markdown = (root / result.report_path).read_text(encoding="utf-8")
        html = (root / result.html_report_path).read_text(encoding="utf-8")
        ...
        raise AttestationEmissionError(
            "persisted distributed evidence is unavailable or invalid"
        ) from error
    if persisted != result:
        raise AttestationEmissionError("persisted distributed result differs from memory evidence")
    if markdown != render_distributed_markdown(persisted) or html != render_distributed_html(
        persisted
    ):
        raise AttestationEmissionError("persisted distributed reports differ from the result")
    ...
    try:
        ...
        raise AttestationEmissionError("distributed checkpoint cannot be fingerprinted") from error
    if checkpoint_fingerprint.logical_bytes != persisted.verified_persisted_bytes:
        raise AttestationEmissionError("distributed checkpoint bytes disagree with result")

    try:
        evidence_manifest = EvidenceManifestV1(entries=collect_evidence_entries(root))
    except (OSError, ValueError) as error:
        raise AttestationEmissionError("distributed evidence inventory cannot close") from error
    evidence_manifest_path = write_json_artifact(
        run_root=root,
        relative_path=EVIDENCE_MANIFEST_PATH,
        value=evidence_manifest,
    ...
    attestation = RecoveryAttestationV1(
        ...
        framework_version=persisted.control[0].torch_version,
