...
def test_two_rank_deepspeed_recovers_after_real_targeted_rank_termination(
    tmp_path: Path,
    target_rank: int,
) -> None:
    run_root = tmp_path / f"deepspeed-rank-{target_rank}-failure"
    ...
    assert result.gate.passed is True
    assert result.gate.failed_check_ids == ()
    assert len(result.gate.checks) == 42
